...
int main() {
  ...
  h.onMessage([&current_car_speed, &map_waypoints_x, &map_waypoints_y, &map_waypoints_s, &map_waypoints_dx, &map_waypoints_dy, &lane](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
	...
	double const DIST_TOO_CLOSE = 30; //30 meters
	double const DIST_TO_FRONT_CAR = 60;
	double const DIST_TO_BACK_CAR = 10;
			...
			*/
			...
			for (int i = 0; i < sensor_fusion.size(); i++)
			{
				...
				if (d < (2 + 4 * lane + 2) && d >(2 + 4 * lane - 2)) // Each lane is 4m wide. So, if car is in lane 1, lane width is from 4m to 8m
				{
					...
					if ((other_car_s > car_s) && ((other_car_s - car_s) < DIST_TOO_CLOSE))  // If Other car's future s value is greater than our car's future s value and distance between them is less than 30m then take action
					{
						...
						if (lane == 0) // left lane
						{
							bool left_lane_car_exists = false;
							for (int j = 0; j < sensor_fusion.size(); j++)
							{
								...
								if (dist_of_other_car < (2 + 4 * 1 + 2) && dist_of_other_car >(2 + 4 * 1 - 2))  // if other cars in center lane
								{
									...
									{
										lane = 1;
										if (current_car_speed > (0.5 * MAX_SPEED) && (check_car_s_other - car_s) < 100) { lane = 0; } // If car is too fast, do not change the lane
									}
									else if ((check_car_s_other < car_s) && ((car_s - check_car_s_other) > DIST_TO_BACK_CAR))
									{
										lane = 1;
										if (current_car_speed > (0.5 * MAX_SPEED) && (car_s - check_car_s_other) < 50) { lane = 0; } // If car is too fast, do not change the lane
									}
									else
									{
										lane = 0; // If one of the cars is very near 
									}
								}
							}
							if (left_lane_car_exists == false)
							{
								lane = 1;    // If there are no cars in center lane, change the lane
							}
						}
						else if (lane == 2) // Right lane
						{
							bool right_lane_car_exists = false;
							for (int j = 0; j < sensor_fusion.size(); j++)
							{
								float dist_of_other_car = sensor_fusion[j][6];
								if (dist_of_other_car < (2 + 4 * 1 + 2) && dist_of_other_car >(2 + 4 * 1 - 2)) // if other cars in center lane
								{
									right_lane_car_exists = true;
									double vx_other = sensor_fusion[j][3];
									double vy_other = sensor_fusion[j][4];
									double check_speed_other = sqrt(vx_other * vx_other + vy_other * vy_other);
									double check_car_s_other = sensor_fusion[j][5];  // s value of the other car
									...
									{
										lane = 1;
										if (current_car_speed > (0.5 * MAX_SPEED) && (check_car_s_other - car_s) < 100) { lane = 2; } // If car is too fast, do not change the lane
									}
									else if ((check_car_s_other < car_s) && ((car_s - check_car_s_other) > DIST_TO_BACK_CAR))
									{
										lane = 1;
										if (current_car_speed > (0.5 * MAX_SPEED) && (car_s - check_car_s_other) < 50) { lane = 2; } // If car is too fast, do not change the lane
									}
									else
									{
										lane = 2; // If one of the cars is very near 
									}
								}
							}
							if (right_lane_car_exists == false)
							{
								lane = 1;  // If there are no cars in center lane, change the lane
							}
						}
