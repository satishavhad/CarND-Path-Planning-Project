...
int main() {
  ...
  h.onMessage([&current_car_speed, &map_waypoints_x, &map_waypoints_y, &map_waypoints_s, &map_waypoints_dx, &map_waypoints_dy, &lane](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
	...
	double const DIST_TOO_CLOSE_BREAK = 20; //30 or 40 meters
	double const DIST_TOO_CLOSE_CHANGE_PATH = 50;
	double const DIST_TO_FRONT_CAR = 20;
	double const DIST_TO_BACK_CAR = 5;
			...
			*/
			...
			for (int i = 0; i < sensor_fusion.size(); i++)
			{
				...
				if (d < (2 + 4 * lane + 2) && d >(2 + 4 * lane - 2)) // Each lane is 4m wide. So, if car is in lane 1, lane width is from 4m to 8m
				{
					...
					if ((other_car_s > car_s) && ((other_car_s - car_s) < DIST_TOO_CLOSE_CHANGE_PATH))  // If Other car's future s value is greater than our car's future s value and distance between them is less than 30m then take action
					{
